...
if [ "$CRUMB" = true ] ; then
    ...
    while IFS=',' read STUDENT GIT_URL; do 
        ...
        curl -s -X POST "${JENKINS_URL}/job/${STUDENT}/job/practice${PRACTICA}_${STUDENT}/createItem?name=${JOB_NAME}" --user ${USER}:${PASS} --data-binary "@/tmp/${JOB_NAME}_${STUDENT}" -H "Content-Type:text/xml" -H "${CRUMB_KEY}"
    done < ${FILE}
else
    while IFS=',' read STUDENT GIT_URL; do 
        # Modify the job template of Jenkins to include the repo of each student
