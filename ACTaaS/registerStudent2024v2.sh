...
usage() { echo "Usage: $0 [-f <students_names_file>] [-j <Jenkins_URL>] [-u <Jenkins_user>] [-p <Jenkins_password>] [-s <single_student_account>] [-c <type>] [-a <number_of_assignments>] [-n <job_name>]" 1>&2; exit 1; }
...
createStudent() {
    ...
    while [ $i -lt ${PRACTICES:-1} ]; do
        i=$((i + 1))
        folder_name="P${i}" # Default folder name
        if [ ! -z "$TYPE" ]; then
            folder_name="P${i}__${TYPE}" # Add type to the folder name
        fi
        if [ ! -z "$JOB_NAME" ]; then
            folder_name="${JOB_NAME}_${i}" # Add job name to the folder name
        fi
    ...
    # Assign global and project roles to the student
...
while getopts ":f:j:a:u:s:p:c:n:" o; do
    case "${o}" in
        f)
            FILE=${OPTARG} #for a list of users
            ;;
        j)
            ...
            ;;
        u)
            USER=${OPTARG} #Jenkin admin user
            ;;
        p)
            PASS=${OPTARG} #Jenkin admin password
            ;;
        s)
            STUDENT=${OPTARG} #for a single student
            ;;
        a)
            PRACTICES=${OPTARG} #number of practices
            ;;
        c)
            TYPE=${OPTARG}
...
if [ -z "${JENKINS_URL}" ] || [ -z "${USER}" ] || [ -z "${PASS}" ]; then
    cecho "BLUE" "ERROR: some parameters are missing, please consider usage."
    usage
    exit 1
fi

if [ ! -f "jenkins-cli.jar" ]; then
    cecho "YELLOW" "ERROR: file jenkins-cli.jar not found."
    exit 1
fi
