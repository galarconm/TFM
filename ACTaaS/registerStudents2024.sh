...
usage() { echo "Usage: $0 [-f <students_names_file>] [-j <Jenkins_URL>] [-u <Jenkins_user>] [-p <Jenkins_password>] [-s <single_student_account>] [-c <type>] [-a <number_of_asssingments>]" 1>&2; exit 1; }
checkURL() {
...
createStudent() {
    ...
    do
        ...
        curl -s -X POST "${JENKINS_URL}/job/${STUDENT}/createItem?name=P${i}__${TYPE}&mode=com.cloudbees.hudson.plugins.folder.Folder&Submit=OK" -H "Content-Type:application/x-www-form-urlencoded" --user ${USER}:${PASS} 
    done
...
if [ -z "${JENKINS_URL}" ] || [ -z "${USER}" ] || [ -z "${PASS}" ] || [ -z "${TYPE}" ] || [ -z "${PRACTICES}" ]; then
